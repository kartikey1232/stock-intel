...
### SEC 6-K results (2026-09-26)

- `collectors/sec_results.py` fetches the Indian results INFY and HDFCBANK furnish to the
  SEC with Form 6-K (EDGAR, 1 request/s per host, User-Agent with `SEC_CONTACT_EMAIL`).
  Ranking XBRL > SEC 6-K > IR PDF; the checklist counts SEC quarters as done. Runs as an
  isolated filings step in `run_update.py`, after the inbox import.
